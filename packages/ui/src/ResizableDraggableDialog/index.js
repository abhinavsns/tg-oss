...
export default class ResizableDraggableDialog extends React.Component {
  componentDidMount() {
    ...
    setTimeout(() => {
      this.setDefaults();
      try {
        const el = this.containerEl.querySelector(".bp6-dialog-body");
  ...
  setDefaults = ({ doNotSetXOrWidth } = {}) => {
    ...
    if (height) {
      ...
      heightToUse = (document.querySelector(".bp6-dialog-body") || {})
        .scrollHeight;
      if (heightToUse) {
  ...
  render() {
    ...
    return (
      <div
        ...
        className="tg-bp6-dialog-resizable-draggable"
        style={{ top: 0, left: 0, position: "fixed" }}
      >
        <Rnd
          cancel=".bp6-dialog-close-button"
          enableResizing={{
            bottomLeft: true,
            bottomRight: true,
            topLeft: true,
