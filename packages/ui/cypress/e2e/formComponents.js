...
describe("formComponents", () => {
  ...
  it(`switch field with beforeOnChange should work showing a confirmation dialog`, () => {
    ...
    cy.contains(`W/ Confirmation`).click();
    cy.contains("Are you sure???");
    cy.contains(".bp6-dialog button", "Cancel").click();
    cy.contains(`W/ Confirmation`).click();
    cy.contains("Are you sure???");
  });
  it(`ReactSelectField works for single select`, () => {
    ...
    cy.get("@inputWrapper").contains(`.bp6-tag`, "Kyle Craft");
    cy.contains("label", "disallowClear").click();
    cy.get(".tg-test-react-select-field .tg-select .bp6-icon-cross").should(
      "not.exist"
    );
    cy.contains("label", "disallowClear").click();
    cy.get("@inputWrapper").find(".bp6-icon-cross").click();
    cy.get("@inputWrapper")
      .contains(`.bp6-tag`, "Kyle Craft")
      .should("not.exist");
  });
  ...
  menu pop up again`, () => {
    ...
    // should stay open
    cy.get(".tg-select-option").should("exist");
    cy.get(".bp6-multi-select-popover").should("exist");
    cy.get("@inputWrapper").find(".bp6-icon-caret-up").click();
    cy.get(".tg-select-option").should("not.exist", { timeout: 10000 });
    cy.get(".bp6-multi-select-popover").should("not.exist");
    cy.get("@inputWrapper")
      .contains(".bp6-tag", "Kyle Craft")
      .should("exist")
      .find(".bp6-icon-small-cross")
      .click();
  ...
  it(`ReactSelectField multi delete keyboard shortcut works as expected`, () => {
    ...
    cy.get("@inputWrapper").parent().contains(".bp6-tag", "Kyle Craft");
    cy.get("@inputWrapper").type("{backspace}{backspace}");
    cy.get("@inputWrapper")
      .parent()
      .contains(".bp6-tag", "Kyle Craft")
      .should("not.exist");
    cy.get(".bp6-multi-select-popover .tg-select-option").should("exist");
    cy.get("@inputWrapper").type("{enter}");
    cy.get("@inputWrapper").parent().contains(".bp6-tag", "Rodrigo Pavez");
  ...
  it(`ReactSelectField multi multiple enter keyboard shortcuts work as expected`, () => {
    cy.get(".tg-test-react-select-field-multi .tg-select input")
      ...
      .type("{enter}{enter}");
    cy.get("@inputWrapper").parent().contains(".bp6-tag", "Rodrigo Pavez");
    cy.get("@inputWrapper").parent().contains(".bp6-tag", "Ximena Morales");
    // cy.get("@inputWrapper").type("{backspace}{backspace}");
    // cy.get("@inputWrapper")
    //   .parent()
    //   .contains(".bp6-tag", "Kyle Craft")
    ...
    //   cy.get("@inputWrapper")
    //   .parent()
    //   .contains(".bp6-tag", "Rodrigo Pavez");
  ...
  it(`isRequired can be passed to any field to make it required!`, () => {
    cy.contains(".bp6-button", "Submit Form").click();
  ...
  it("disabled react select field should not be editable", () => {
    ...
    cy.get(
      ".tg-test-react-select-field-disabled .tg-select-value .bp6-tag-remove"
    ).should("not.exist");
