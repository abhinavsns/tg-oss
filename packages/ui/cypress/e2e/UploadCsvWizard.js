...
describe("UploadCsvWizard.spec", () => {
  it(`wizard should let a "perfect" file through without any additional steps. Users should be able to edit it after the fact`, () => {
    ...
    cy.get(`.bp6-disabled:contains(Edit Data)`);
    cy.focused().type(` tom{enter}`);
    cy.get(`.bp6-button:contains(Edit Data)`).click();
    cy.contains(`File Updated`);
    cy.get(`.tg-upload-file-list-item-edit`).click();
    cy.get(
      `[data-index="4"] [data-test="tgCell_sequence"]:contains(tom)`
    ).click();
    cy.get(`.bp6-button:contains(Cancel)`).click();
  ...
  it(`messed up headers should trigger the wizard. editing the added file should work`, () => {
    ...
    cy.contains(".bp6-dialog", `zonk`).should("not.exist"); //the data from the file should be previewed
    cy.get(".bp6-dialog tr:contains(prescription):contains(Description)"); //the matched headers should show up
    cy.get(".bp6-dialog tr:contains(Name):contains(tame)"); //the matched headers should show up
    cy.get(".bp6-dialog tr:contains(Match Type):contains(type2)"); //the matched headers should show up
    cy.get(".bp6-dialog tr:contains(Is Regex):contains(typo)").should(
      "not.exist"
    );

    cy.get(`.tg-test-is-regex`).click();
    cy.contains(".tg-select-option", "typo").click();
    cy.contains(".bp6-dialog", `zonk`).should("exist"); //the data from the file should be previewed
...
a,,desc,,false,dna,misc_feature
  ...
  it(`async promise accept should still allow the schema to be updated dynamically (changing )`, () => {
    ...
    cy.get(`button:contains(Add File).bp6-disabled`);
    cy.contains(`Cancel`).click();
    cy.contains(`File Upload Aborted`);
    cy.get(`.bp6-dialog`).should("not.exist");
  });
  ...
  it(`isUnique should work as a validation rule on the table for editing, pasting, undo/redo`, () => {
    ...
    cy.get(`button:contains(Add File).bp6-disabled`).should("not.exist");
    const IS_LINUX =
  ...
  it(`going back and forth between the pages should not clear the data that has been changed unless the column was switched`, () => {
    ...
    cy.get(`.tg-test-sequence .bp6-icon-cross`).click();
    ...
    cy.get(`.bp6-button:contains(Yes)`).click();
    cy.contains("Review and Edit Data").click();
    cy.get(
      `.hasCellError[data-tip="Please enter a value here"] [data-test="tgCell_name"]:first`
    ).should("exist");
  ...
  it(`multiple csv files packed into a zip should bring up a wizard with a tab for every file. They should be able to be edited after`, () => {
    ...
    cy.get(
      `.bp6-dialog .bp6-tab[aria-selected="true"]:contains(testUploadWizard_invalidData.csv) .bp6-icon-warning-sign`
    );
    cy.get(
      `.bp6-dialog .bp6-tab[aria-selected="true"]:contains(testUploadWizard_messedUpHeaders.csv) .bp6-icon-warning-sign`
    ).should("not.exist");
    cy.get(
      ".bp6-dialog .bp6-tab:contains(testUploadWizard_messedUpHeaders.csv) .bp6-icon-warning-sign"
    );
    cy.get(
      ".bp6-dialog .bp6-tab:contains(testUploadWizard_invalidDataNonUnique.csv) .bp6-icon-tick-circle"
    );
    cy.get(
      ".bp6-dialog .bp6-tab:contains(testUploadWizard_nearPerfect.csv) .bp6-icon-tick-circle"
    );
    cy.contains(".bp6-dialog", `zonk`);
    cy.contains(".bp6-dialog", `DEscription`); //the matched headers should show up
    cy.contains(".bp6-dialog", `Description`); //the expected headers should show up
    cy.contains("Review and Edit Data").click();
    ...
    cy.get(
      ".bp6-dialog tr:contains(Sequence BPs) .bp6-multi-select-tag-input-input"
    ).click();
    cy.get(".bp6-menu-item:contains(lolz)").click();
    // eslint-disable-next-line cypress/no-unnecessary-waiting
    cy.wait(200);
    cy.get(
      ".bp6-dialog tr:contains(Name) .bp6-multi-select-tag-input-input"
    ).click();
    cy.get(".bp6-menu-item:contains(typo)").click();

    cy.contains("Review and Edit Data").click();
    cy.get(".bp6-button:contains(Add 10 Rows)").eq(1).click();
    cy.get(`.bp6-button:contains(Finalize Files)`).eq(1).click();
...
a,,g,false,dna,misc_feature`,
    ...
    cy.get(`.bp6-dialog tr:contains(Name) .tg-select-clear-all`).click();
    cy.contains("Review and Edit Data").click();
    cy.get(
      `.bp6-dialog .bp6-tab[aria-selected="true"]:contains(test.csv) .bp6-icon-warning-sign`
    );
    cy.get(`.bp6-dialog .bp6-tab:contains(test2.csv) .bp6-icon-warning-sign`);
    cy.get(`.bp6-dialog .bp6-tab:contains(test3.csv) .bp6-icon-warning-sign`);
    ...
    cy.get(".bp6-menu-item:contains(name)").click();
    cy.contains(
      `Are you sure you want to edit the columm mapping? This will clear any changes you've already made to the table data`
    );
    cy.get(`.bp6-button:contains(Yes)`).click();
    ...
    cy.get(`.bp6-dialog .bp6-tab:contains(test2.csv) .bp6-icon-warning-sign`);
    cy.get(`.bp6-dialog .bp6-tab:contains(test3.csv) .bp6-icon-tick-circle`);

    cy.get(`[data-tip="Please enter a value here"]`);
    cy.get(`.hasCellError:first [data-test="tgCell_name"]`);
    cy.get(`button:contains(Next File).bp6-disabled`);
    cy.get(`.hasCellError:first [data-test="tgCell_name"]`).click({
      force: true
    });
    cy.focused().type(" haha{enter}");
    ...
    cy.get(`button:contains(Next File):first`).click();
    cy.get(`.bp6-spinner`);
    cy.get(`.bp6-spinner`).should("not.exist");
    cy.get(`.hasCellError:first [data-test="tgCell_name"]`).click({
      force: true
    });
...
a,,g,false,dna,misc_feature`,
    ...
    cy.get(".bp6-menu-item:contains(lalala)").click();
    cy.get(".bp6-dialog tr:contains(Name):contains(lalala)").click();
    cy.get(
      ".bp6-dialog tr:contains(Sequence BPs) .bp6-multi-select-tag-input-input"
    ).click();
    cy.get(".bp6-menu-item:contains(zonk)").first().click();
    cy.contains("Review and Edit Data").click();
...
`,
    ...
    cy.contains(`.bp6-dialog .bp6-button`, "Review and Edit Data").click();
    cy.get(`[data-tip="Please enter a value here"]`).click();
    cy.focused().type(" haha{enter}");
    cy.contains(`.bp6-dialog .bp6-button`, "Add File").click();
...
thomas,,g,false,dna,misc_feature`,
    ...
    cy.contains(`Cannot be Thomas`);
    cy.contains("Review and Edit Data").click();
    cy.get(`.bp6-button.bp6-disabled:contains(Next File)`);
    cy.get(`[data-tip="Cannot be Thomas"]`);
    cy.get(`.hasCellError:first [data-test="tgCell_name"]`).click({
      force: true
    });
    ...
    cy.get(`.bp6-spinner`);
    cy.get(`.bp6-spinner`).should("not.exist");
    cy.get(`.bp6-button.bp6-disabled:contains(Finalize Files)`);
    cy.get(`[data-tip="Cannot be Thomas"]`);
    cy.get(`.hasCellError:first [data-test="tgCell_name"]`).click({
      force: true
    });
    cy.focused().type(" ba{enter}");
    cy.get(`.bp6-button:contains(Finalize Files):last`).click();
  });
  it(`manual entry file name should be editable. The name should be able to be edited after`, () => {
    ...
    cy.contains(".bp6-button", "Add File").click();
  ...
  it(`multiple manual entries should get unique names. They should be able to be edited after`, () => {
    ...
    cy.focused().type(`{backspace}`);
    cy.get(`.bp6-disabled:contains(Edit Data)`).should("not.exist");
    cy.focused().type(` tom{enter}`);
  ...
  it(`zip data that contains one or more messed up CSV/XLSX should trigger the wizard`, () => {
    ...
    cy.contains(`It looks like there was an error with your data`);
    cy.contains(".bp6-dialog", `zonk`); //the data from the file should be previewed
    cy.contains(".bp6-dialog", `DEscription`); //the matched headers should show up
    cy.contains(".bp6-dialog", `Description`); //the expected headers should show up
    cy.contains("Review and Edit Data").click();
    cy.get(`[data-tip="Please enter a value here"]`);
  ...
  it(`manual entry should work, additional untouched (_isClean) rows should be omitted`, () => {
    ...
    cy.get(`[data-test="Is Regex"] .bp6-checkbox`);
