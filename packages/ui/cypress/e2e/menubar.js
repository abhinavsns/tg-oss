...
describe("MenuBar", () => {
  ...
  it(`component:()=>{} item in a menubar can be searched!`, () => {
    cy.contains(".bp6-button-text", "Help").click();
    cy.focused().type("melting");
    cy.get(
      `.bp6-menu-item:contains(Melting Temp of Selection) .bp6-icon-small-tick`
    );
    cy.focused().clear().type("component cmd example");
    cy.get(
      `.bp6-menu-item:contains(component cmd example):contains(File) .bp6-key-combo`
    );
  });
  it(`hiddenButSearchable text in a menubar can be searched!`, () => {
    cy.contains(".bp6-button-text", "Help").click();
    cy.focused().type("hidden but");
    cy.get(`.bp6-menu-item:contains(Cmd With Ticks)`);
  });
  it(`menubar can be searched!`, () => {
    ...
    cy.contains(
      ".tg-menu-search-suggestions .bp6-menu-item.bp6-disabled",
      "I'm disabled"
    ).should("exist");
    // sub menus should be accessible on hover!
    cy.focused().type("{selectall}other");
    cy.contains(".tg-menu-search-suggestions .bp6-menu-item", "Other").trigger(
      "mouseover"
    );
    cy.contains(".bp6-menu-item", "XXXXX").click({ force: true });

    const closeToasts = () => {
      cy.get(".bp6-toast .bp6-icon-cross").each(el => {
        el.click();
      });
    };
    closeToasts();
    ...
    cy.get(".tg-menu-search-suggestions .bp6-menu-item").should(
      "have.length",
      10
    );
    ...
    cy.contains(".tg-menu-search-suggestions .bp6-menu-item", "Long React")
      .find("input")
      .type("ha");
    ...
    cy.contains(".bp6-menu-item", "Don't Dismiss").click();
    cy.contains(".bp6-toast", "This menu's not going away any time soon");
    cy.get(`.tg-menu-bar-help-search input`).focus();

    cy.triggerFileCmd("{selectall}React", { noEnter: true, noOpen: true });
    cy.contains(".bp6-menu-item", "ReactText 9").click();
    cy.contains(".bp6-toast", "Fired ReactText!");

    //it can get the File > ReactText path correctly
    closeToasts();
    cy.triggerFileCmd("hel", { noEnter: true });
    cy.contains(".bp6-menu-item", "File > ReactText");

    //menu items with hideFromMenuSearch should not show up
    cy.triggerFileCmd("{selectall}About", { noEnter: true, noOpen: true });

    cy.contains(".bp6-menu-item-label", "Help").should("not.exist");

    //dynamic cmd created submenus should be visible
    cy.triggerFileCmd("{selectall}cmd", { noEnter: true, noOpen: true });
    cy.contains(".bp6-menu-item", "Cmd Submenu");
    cy.contains(".bp6-menu-item", "File > ReactText");
