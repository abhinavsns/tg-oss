describe("tgSelect", () => {
  ...
  it(`isTagSelect will only allow a single tag option to be selected at a time`, () => {
    ...
    cy.contains(".bp6-tag-input-values", "my: friend");

    cy.get(".tg-select input").type("my accomplice{enter}");
    cy.contains(".bp6-tag-input-values", "my: friend").should("not.exist");
    cy.contains(".bp6-tag-input-values", "my: accomplice").should("exist");
  });
  ...
  it(`creatable won't allow for making duplicates`, () => {
    ...
    cy.contains(".bp6-menu-item", `Create "tHEr"`);
    cy.get(".tg-select input").type("e");
    cy.contains(".tg-select-option", "There");
    cy.contains(".bp6-menu-item", `Create "tHEr"`).should("not.exist");
  });
  it(`creatable multi select won't allow for making duplicates`, () => {
    ...
    cy.contains(".bp6-menu-item", `Create "hey"`).should("not.exist");
    cy.get(".tg-select input").type("a");
    cy.contains(".bp6-menu-item", `Create "heya"`).should("exist");
  });
  ...
  it(`onCreateNewOption should trigger and create a new option`, () => {
    ...
    cy.contains(".bp6-menu-item", `Create "weeeeee"`).click();
    cy.contains(".bp6-toast", "New option weeeeee created").should("exist");
