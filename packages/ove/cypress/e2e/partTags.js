describe("partTags", function () {
  ...
  it(`the part tag search dropdown should only show existing tags on parts as options to search`, () => {
    ...
    cy.get(`[data-test="partToolDropdown"]`).click();
    cy.focused().click();
    cy.contains(".bp6-tag", "zoink");
    cy.get(`.bp6-tag:contains("tag2")`).should("have.length", 1);
    cy.contains(".bp6-tag", "status: ready");
    cy.contains(".bp6-tag", "status: broken").should("not.exist");
    cy.contains(".bp6-tag", "something else").should("not.exist");
  });
  ...
  it(`should be able to see and search for part tags in the properties window`, () => {
    ...
    cy.contains(".rt-tr .bp6-tag", "status: ready").should("exist");
    // eslint-disable-next-line cypress/no-unnecessary-waiting
    cy.wait(0);
    cy.get(".datatable-search-input").type("zoink{enter}");
    cy.contains(".rt-tr .bp6-tag", "status: ready").should("not.exist");
    cy.contains(".rt-tr .bp6-tag", "zoink").should("exist");
  });
  ...
  it(`should be able to add/edit tags on parts`, () => {
    ...
    cy.contains(".bp6-menu-item", "status: ready").click();
    cy.contains(".bp6-tag-input-values", "status: ready").should("exist");
    cy.contains(".bp6-menu-item", "status: broken").click();
    cy.contains(".bp6-tag-input-values", "status: ready").should("not.exist");
    cy.contains(".bp6-menu-item", "tag2").click();
    ...
    cy.contains(".bp6-tag-input-values", "status: broken").should("exist");
