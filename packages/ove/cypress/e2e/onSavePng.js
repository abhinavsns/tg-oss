describe("onSavePng", function () {
  ...
  it(`generate a png onSave if pngGenerate option is set to true`, () => {
    ...
    cy.contains(".bp6-dialog", "Generating Image to Save");
    cy.get(".bp6-dialog .veCircularView");
    ...
    cy.get(`[data-test="veStatusBar-circularity"]`)
      .find("select")
      .select("Linear");
    cy.contains(".bp6-dialog button", "Truncate Annotations").click();

    cy.get(`[data-test="saveTool"]`).click();
    cy.contains(".bp6-dialog", "Generating Image to Save");
    cy.get(".bp6-dialog .veLinearView");
