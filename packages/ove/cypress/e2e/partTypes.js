describe("partTypes", function () {
  ...
  it("should be able to make a part from feature", () => {
    ...
    cy.contains(".bp6-menu-item", "Make a Part from Feature").click();
    cy.get(".bp6-menu").should("not.exist");
    cy.contains(".veRowViewPart title", "test feature").trigger("contextmenu", {
      force: true
    });
    cy.contains(".bp6-menu-item", "Edit Part", { timeout: 15000 }).click();
    cy.get(".bp6-dialog-body").contains(".tg-select", "misc_RNA");
  });
});
