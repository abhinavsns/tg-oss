describe("orfs", function () {
  it("the minimum orf size input should work as expected!", function () {
    ...
    cy.contains(".bp6-menu-item:contains(7)", "ORFs").click();
    cy.get(".bp6-menu-item:contains(Minimum ORF Size) input").should(
      "have.value",
      "300"
    );
    //negative numbers shouldn't be allowed and users should be able to select all!
    cy.get(".bp6-menu-item:contains(Minimum ORF Size) input").type(
      "{selectall}-7000",
      { force: true }
    );
    cy.get(".bp6-menu-item:contains(Minimum ORF Size) input").should(
    ...
    cy.contains(".bp6-menu-item:contains(0)", "ORFs");
  });
  ...
  it("'Use GTG And CTG As Start Codons' checkbox status should synchronize between toolbar and properties table", function () {
    ...
    cy.contains(".bp6-menu-item:contains(7)", "ORFs").click();
    cy.get(
      ".bp6-menu-item:contains(Use GTG And CTG As Start Codons) .bp6-icon-blank"
    ).should("exist");
    cy.get(".bp6-menu-item:contains(Use GTG And CTG As Start Codons)").click({
      force: true
    });
    cy.get(
      ".bp6-menu-item:contains(Use GTG And CTG As Start Codons) .bp6-icon-small-tick"
    ).should("exist");
  ...
  it("'Use GUG And CUG As Start Codons' should show for RNA sequence", function () {
    ...
    cy.contains(".bp6-menu-item:contains(7)", "ORFs").click();
    cy.get(
      ".bp6-menu-item:contains(Use GUG And CUG As Start Codons) .bp6-icon-blank"
    ).should("exist");
  });
  ...
  it("ORFs should have Size AA equals (sequence size / 3 - 1)", function () {
    ...
    cy.get(".propertiesVisFilter").click();
    cy.get(".bp6-menu-item:contains(ORFs):contains(7)").click({
      force: true
    });
    cy.get(".bp6-menu-item:contains(ORFs):contains(7)").click({
      force: true
